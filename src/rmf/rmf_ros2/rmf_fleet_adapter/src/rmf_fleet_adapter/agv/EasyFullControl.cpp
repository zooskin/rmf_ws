...
// ==================================================================
// [PATCH] final_name accessor: plan의 최종 목적지 waypoint 이름
// ==================================================================
std::string EasyFullControl::Destination::final_name() const
{
  return _pimpl->final_name;
}
// ==================================================================
// [PATCH] waypoint_names accessor: planned_path의 전체 waypoint 이름 목록
// ROS topic 대신 Destination 객체로 직접 전달하여 DDS 타이밍 이슈 제거
// ==================================================================
std::vector<std::string> EasyFullControl::Destination::waypoint_names() const
{
  return _pimpl->waypoint_names;
}
// ==================================================================
  ...
  RequestCompleted path_finished_callback_)
{
  ...
  // ==================================================================
  // [PATCH] plan의 최종 목적지 이름: RobotContext에서 가져온다.
  // ExecutePlan::make()에서 Goal로부터 계산하여 저장한 값이다.
  // ==================================================================
  std::string plan_final_name;
  if (context)
  {
    plan_final_name = context->plan_final_name();
  }
  if (plan_final_name.empty() && !waypoints.empty())
  {
    plan_final_name = nav_params->get_vertex_name(
      graph, waypoints.back().graph_index());
  }
  // ==================================================================
  ...
  // ==================================================================
  // [PATCH] Build waypoint_names for Destination (replaces ROS topic)
  // ==================================================================
  std::vector<std::string> plan_waypoint_names;
  {
    ...
    for (const auto& wp : waypoints)
    {
      ...
      if (prev_idx.has_value() && *prev_idx == idx)
        continue;
      if (prev_idx.has_value()
          && nav_params->in_same_stack(*prev_idx, idx))
        continue;
      prev_idx = idx;
      const std::string name =
        nav_params->get_vertex_name(graph, wp.graph_index());
      if (!name.empty())
        plan_waypoint_names.push_back(name);
    }
  }
  // ==================================================================
  ...
  while (i1 < waypoints.size())
  {
    ...
    // ==================================================================
    // [PATCH] plan_final_name + plan_waypoint_names을 Destination에 전달
    // ==================================================================
  ...
  if (!nav_params->skip_rotation_commands && !queue.empty())
  {
    ...
    if (course_vector.norm() > nav_params->max_merge_waypoint_distance)
    {
      ...
      if (yaw_cost > 5.0*M_PI/180.0)
      {
        ...
        // ==================================================================
        // [PATCH] plan_final_name + plan_waypoint_names (rotation command)
        // ==================================================================
        auto destination = EasyFullControl::Destination::Implementation::make(
          initial_map,
          command_position,
  ...
  if (initial_map != reported_location->map)
  {
    ...
    // ==================================================================
    // [PATCH] plan_final_name + plan_waypoint_names (relocalize)
    // ==================================================================
  ...
  RequestCompleted docking_finished_callback_)
{
    ...
    std::abs(rmf_utils::wrap_to_pi(angle - current_pose[2])) > 1e-2)
  {
    ...
    // ==================================================================
    // [PATCH] dock rotation: final_name + waypoint_names 추가 (빈 값)
    // ==================================================================
  ...
  // ==================================================================
  // [PATCH] dock command: final_name + waypoint_names 추가 (빈 값)
  // ==================================================================
  auto destination = EasyFullControl::Destination::Implementation::make(
    wp1.get_map_name(),
    command_position,
